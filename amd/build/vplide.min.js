 ...
 */
define(['jquery', 'jqueryui', 'mod_vpl/vplutil', 'mod_vpl/vplidefile', 'mod_vpl/vplidebutton', 'mod_vpl/vplterminal', 'mod_vpl/vplvnc'], function ($, jqui, VPLUtil, VPLFile, VPLIDEButtons, VPLTerminal, VPLVNCClient) {
  ...
  var VPLIDE = function (rootId, options) {
    ...
    var str = VPLUtil.str;
    var rootObj = $('#' + rootId);
    $("head").append('<meta name="viewport" content="initial-scale=1">').append('<meta name="viewport" width="device-width">').append('<link rel="stylesheet" href="../editor/VPLIDE.css"/>');
    ...
    function FileManager() {
      ...
      this.renameFile = function (oldname, newname, showError) {
        ...
        try {
          ...
          if (!VPLUtil.validPath(newname) || fileNameIncluded(newname) || twoBlockly(oldname, newname)) {
    ...
    this.updateEvaluationNumber = function (res) {
      if (typeof res.nevaluations != 'undefined') {
        ...
        if (typeof res.reductionbyevaluation != 'undefined' && res.reductionbyevaluation > '' && res.reductionbyevaluation != 0) {
    ...
    this.setResult = function (res, go) {
      ...
      if (hasContent && typeof MathJax == 'object') {
    ...
    function renameHandler(event) {
      ...
      fileManager.renameFile(fileManager.currentFile('getFileName'), $('#vpl_ide_input_renamefilename').val(), showErrorMessage);
      event.preventDefault();
    }
    ...
    function resetFiles() {
      VPLUtil.requestAction('resetfiles', '', {}, options.ajaxurl).done(function (response) {
      ...
      originalAction: function () {
        ...
        function doSave() {
          ...
          if (!langConfigParseError) {
            VPLUtil.requestAction('save', 'saving', data, options.ajaxurl).done(function (response) {
    ...
    function executionRequest(action, acting, data) {
      ...
      if (!lastConsole.isConnected()) {
        VPLUtil.requestAction(action, '', data, options.ajaxurl).done(function (response) {
    ...
    var alwaysActive = ['filelist', 'more', 'fullscreen', 'about', 'resetfiles', 'download', 'comments', 'console', 'import', 'fontsize', 'timeleft'];
    ...
    executionActions = {
      ...
      'run': function (content, coninfo, ws) {
        ...
        if (type == 'terminal' || type == 'webterminal') {
          ...
          VNCClient.connect(coninfo.secure, coninfo.server, coninfo.portToUse, coninfo.VNCpassword, coninfo.executionPath, function () {
            ws.close();
            focusCurrentFile();
          });
    ...
    fileManager.resetModified();
    VPLUtil.requestAction('load', 'loading', options, options.loadajaxurl).done(function (response) {
      var allOK = true;
      var files = response.files;
