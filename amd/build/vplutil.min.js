...
define(['jquery', 'jqueryui', 'core/log'], function ($, jqui, log) {
  ...
  VPLUtil.readZipFile = function (data, save, progressBar, end) {
    ...
    if (typeof JUnzip == 'undefined') {
      VPLUtil.loadScript(['../editor/zip/inflate.js', '../editor/zip/unzip.js'], function () {
  ...
  VPLUtil.webSocketMonitor = function (coninfo, title, running, externalActions) {
    ...
    var messageActions = {
      ...
      'retrieve': function () {
        var data = {
          "processid": VPLUtil.getProcessId()
        };
        pb.close();
        delegated = true;
        VPLUtil.requestAction('retrieve', '', data, externalActions.ajaxurl).done(function (response) {
          deferred.resolve();
          if (externalActions.setResult) {
    ...
    ws.onerror = function (event) {
      ...
      if (coninfo.secure && ws.notOpen) {
        VPLUtil.requestAction('getjails', 'retrieve', {}, externalActions.ajaxurl).done(function (response) {
  ...
  VPLUtil.directRunTest = function (URL, command, data) {
    var files = [{
      ...
      name: 'b.c',
      contents: 'int f(){return 1;}',
      encoding: 0
    }];
    VPLUtil.directRun(URL, command, files).done(function (result) {
      var mcount = 0;
      ...
      switch (line.type) {
        ...
        case "caseEnhanced":
          ...
          if (end != null) {
            ...
            tooltipHtml = `<div class="${regCaseOriginal.class}" id="enhancedPop${idPop}" style="display: none; left: 0px; top: 0px;">` + end[0].substring(14, end[0].length) + '</div>';
          }
          var splitBr = line.line.split('<br>');
          splitBr.forEach(str => {
            var sanitizedLine = VPLUtil.sanitizeText(str);
            html += `<div class="${line.class}"` + (end ? ` onmouseover="enhancedMouseOver(event, ${idPop})"` + ` onmousemove="enhancedMouseMove(event, ${idPop})"` + ` onmouseout="enhancedMouseOut(event, ${idPop})"` : '') + '>' + `${sanitizedLine}</div>` + tooltipHtml;
  ...
  (function () {
    ...
    SubmissionHighlighter.prototype.highlightBlockly = function (preid) {
      VPLUtil.loadScript(['../editor/blockly/blockly_compressed.js', '../editor/blockly/msg/js/en.js', '../editor/blockly/blocks_compressed.js'], function () {
        var tag = $('#' + preid);
        var c = tag.html();
        $('#' + preid + 'load').remove();
        tag.html('');
    ...
    SubmissionHighlighter.prototype.highlight = function () {
      ...
      if (needAce && typeof ace === 'undefined') {
        VPLUtil.loadScript(['../editor/ace9/ace.js'], function () {
          self.highlight();
        });
        return;
      }
      ...
      for (var ri = 0; ri < results.length; ri++) {
        ...
        tag.innerHTML = VPLUtil.processResult(text, shFileNames, shFiles, results[ri].noFormat, results[ri].folding);
      }
    };
