--https://gist.github.com/imliam/7a5ed398531a6338081a7f3c25c934fb
...
function lib_lua.scandir(directory, recursive, extensions)
...
--   len: length of file
--   asserts on error
function lib_lua.length_of_file(filename)
...
function lib_lua.file_exists(path)
...
-- on error.
-- Requires Lua 5.1.
function lib_lua.seek(fh, ...)
  assert(fh:seek(...))
end

-- Read an entire file.
-- Use "a" in Lua 5.3; "*a" in Lua 5.1 and 5.2
function lib_lua.readall(filename)
  local fh = assert(io.open(filename, "rb"))
...
function lib_lua.write(filename, contents)
...
function lib_lua.modify(filename, modify_func)
...
function lib_lua.stringify(obj, as_key)
...
lib_lua.null = {}  -- This is a one-off table to represent the null value.

function lib_lua.parse(str, pos, end_delim)
  pos = pos or 1
  ...
  if first == '{' then  -- Parse an object.
    ...
    pos = pos + 1
    while true do
      key, pos = lib_lua.parse(str, pos, '}')
      if key == nil then return obj, pos end
      if not delim_found then error('Comma missing between object items.') end
      pos = skip_delim(str, pos, ':', true)  -- true -> error if missing.
      obj[key], pos = lib_lua.parse(str, pos)
